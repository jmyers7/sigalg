...
class ProbabilitySpace(SigmaAlgebraMethods, ProbabilityMeasureMethods):
    ...
    sig_alg : SigmaAlgebra | None, default=None
        Sigma-algebra defining measurable events. If `None` but a `prob_measure` is given, the sigma-algebra from the probability measure will be used. If `None` and no `prob_measure` is given, a power set sigma-algebra will be created on the sample space.
    prob_measure : ProbabilityMeasure | None, default=None
        Probability measure on the sigma-algebra. If `None` but a `sig_alg` is given, a uniform probability measure will be created on the sigma-algebra. If `None` and no `sig_alg` is given, a uniform probability measure will be created on the power set sigma-algebra of the sample space.
    ...
    * Probability measure 'P':
    ...
    def __init__(
        ...
        if sample_space is not None:
            ...
            if sig_alg is not None and prob_measure is None:
                prob_measure = ProbabilityMeasure.uniform(sig_alg)
            if sig_alg is None and prob_measure is None:
    ...
    def sample_space(self, sample_space: SampleSpace) -> None:
        ...
        This method will clear any existing sigma-algebra or probability measure and set them to the power-set sigma-algebra and uniform probability measure on the new sample space, respectively.
        ...
        if sample_space is not None:
            ...
            sig_alg = SigmaAlgebra.power_set(sample_space)
            self._prob_measure = ProbabilityMeasure.uniform(sig_alg)
        self._sample_space = sample_space
    ...
    def sig_alg(self) -> SigmaAlgebra:
        ...
        2             1
        """
        return self.prob_measure.sig_alg
