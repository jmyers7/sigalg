...
class Set(Index):
    ...
    Extract a set from a domain and print it.
    ...
    def cartesian_product(
        ...
        """Form the Cartesian product of a list of sets.
        ...
        Define a domain and extract two subsets.

        >>> X = Domain.from_sequence(size=3)
        >>> U = Set([0, 1], domain=X, name="U")
        >>> V = Set([1, 2], domain=X, name="V")
        >>> product = Set.cartesian_product([U, V])
    ...
    def lattice(self) -> Lattice:
        ...
        Perform another measurability check, inspect the `lattice` attribute to see the updated number of sigma-algebras, and print the atom data.
        ...
        Notice that the set is not measurable with respect to the third sigma-algebra. The measurability check accordingly returns `False`, and the contents of `lattice` are not changed.
    ...
    def generated_sig_alg(self) -> SigmaAlgebra | None:
        ...
        Define a domain and extract a subset.

        >>> X = Domain.from_sequence(size=3)
        >>> A = Set([0, 1], domain=X)

        Print the sigma-algebra generated by the set. Notice that there are only two atoms with identifiers `0` and `1`. Points that are in the set get an identifier of `1`, while points in the complement have identifier `0`. In other words, the atom identifier mapping is just the indiciator function of the set.

        >>> print(A.generated_sig_alg)  # doctest: +NORMALIZE_WHITESPACE
    ...
    def indicator_atom_data(self, sig_alg: SigmaAlgebra) -> pd.Series | None:
        """Given a sigma-algebra containing the set, get a `pd.Series` indicating the atoms whose union is the set.
        ...
        atom_data : pd.Series | None
            A `pd.Series` indicating the atoms of the sigma-algebra whose union is the set.

        Examples
        --------
        >>> from sigalg.core import Domain, Set, SigmaAlgebra

        Define a domain and a sigma-algebra.
        ...
        Notice that `U` is the atom of `F` with identifier `1`. This will be revealed through the `indicator_atom_data`.

        >>> print(U.indicator_atom_data(F))  # doctest: +NORMALIZE_WHITESPACE
           I_U
        F
        0    0
        1    1
        ...
        Notice that `V` is the union of the two atoms of `F` with identifiers `1` and `2`.

        >>> print(V.indicator_atom_data(F))  # doctest: +NORMALIZE_WHITESPACE
           I_V
        F
        0    0
        1    1
        ...
        if self.indicator_data is not None:
            if self not in sig_alg:
                raise NonMeasurableError(
                    "The set is not measurable with respect to the sigma-algebra."
                )
