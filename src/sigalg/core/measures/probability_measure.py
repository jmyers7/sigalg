...
class ProbabilityMeasure(Measure):
    ...
    def cross_entropy(
        ...
        -------
        cross_entropy : Real
            The cross entropy from this measure to another.
        ...
        Define two probability measures and a base measure for cross entropy. Notice that the sigma-algebra has null atoms.
        ...
        ...         3: 2,
        ...         4: 3,  # P-null atom
        ...         5: 4,  # P-, Q-, and mu-null atom
        ...         6: 4,  # P-, Q-, and mu-null atom
        ...     },
        ... )
        >>> P = ProbabilityMeasure(
        ...     domain=F,
        ...
        if not self.is_absolutely_continuous(
            other, tol=tol
        ) or not other.is_absolutely_continuous(base_measure, tol=tol):
            raise ValueError(
    ...
    def divergence(
        ...
        -------
        kl : Real
            The KL divergence from this measure to another.
        ...
        Define two probability measures for KL divergence. Notice that the sigma-algebra has null atoms.
        ...
        ...         3: 2,
        ...         4: 3,  # P-null atom
        ...         5: 4,  # P- and Q-null atom
        ...         6: 4,  # P- and Q-null atom
        ...     },
        ... )
        >>> P = ProbabilityMeasure(
        ...     domain=F,
        ...
        ...     },
        ... )
        >>> np.allclose(P.divergence(Q), P.cross_entropy(Q, mu) - P.entropy(mu))
        True

        Notes
        -----
        Let $(\Omega, \mathcal{F})$ be a finite measurable space and $P$ and $Q$ two probability measures with $P \ll Q$. The *Kullback Leibler (KL) divergence* from $P$ to $Q$ is the number
    ...
    def mutual_info(
        ...
        Parameters
        ----------
        first_variables : list[Hashable] | None, default=None
        ...
        -------
        mutual_info : Real
            The mutual information of this probability measure.
        ...
        Define a probability measure on a Cartesian product.

        >>> Omega = SampleSpace.cartesian_power([0, 1], n=2)
        >>> P = ProbabilityMeasure(
        ...     domain=Omega,
        ...     mapping={
        ...         (0, 0): 0.15,
        ...
        Define a second measure on the same space and compute its mutual information.
        ...
        Since the mutual information is `0.0`, this measure is the tensor product of its two marginals.

        Now, define a measure on a `3`-fold Cartesian product.

        >>> S = SampleSpace.cartesian_power(
        ...     [0, 1], n=3, variable_names=["s_0", "s_1", "s_2"], name="S"
        ... )
        >>> R = ProbabilityMeasure(
        ...     domain=S,
        ...     mapping={
        ...         (0, 0, 0): 0.2,
        ...         (0, 0, 1): 0.0,
        ...
        >>> R.data.groupby("s_0").sum()
        s_0
        0    0.4
        1    0.6
        Name: R, dtype: float64

        The second marginal has the following data.

        >>> R.data.groupby(["s_1", "s_2"]).sum()
        s_1  s_2
        0    0      0.50
             1      0.15
        1    0      0.15
