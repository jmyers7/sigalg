...
class PoissonProcess(StochasticProcess):
    ...
    Poisson process 'X':
    t       0.0000  1.5625  3.1250  4.6875  6.2500
    omega
    ...
    ...     empirical_probabilities.domain
    ... )
    >>> np.allclose(
    ...
    _repr_name = "PoissonProcess"
    _str_name = "Poisson process"
    ...
    def generate(
        ...
        mode : Literal["enum", "sim"], default="sim"
            This parameter is included only for consistency with the parent class `StochasticProcess`. It is ignored in this implementation, as the Poisson process is always generated by Monte Carlo simulation.
        ...
        info : dict[str, object]
            A dictionary containing the names of the PoissonProcess-specific parameters and their values.
        ...
        if mode == "enum":
            raise ValueError(
                "The Poisson process is always generated by Monte Carlo simulation. "
                "The 'enum' mode is not supported."
            )

        return {"rate": rate, "max_count": max_count}

    # --------------------- properties --------------------- #

    @property
    def rate(self) -> Real:
        """The rate (lambda) of the Poisson process.
    ...
    def max_count(self) -> int:
        """The maximum count of events to simulate.
    ...
    def _simulation_subclass_hook(self) -> pd.DataFrame:
        ...
        self._index = Time(
            self._index.data[self._index <= shortest_complete_arrival_time + 1e-3]
        )
        ...
        trajectories.columns = self._index.data
