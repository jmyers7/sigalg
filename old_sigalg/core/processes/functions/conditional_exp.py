# from ..base import ProcessWithProbabilitySpace


# def conditional_exp(
#     process: ProcessWithProbabilitySpace,
#     RV: callable,
#     sigma_algebra: list,
...
#     """
#     if (
#         not isinstance(process, ProcessWithProbabilitySpace)
#         or process.omega is None
#         or "p" not in process.omega.columns
#     ):
#         raise ValueError(
#             "The process must have a properly set up sample space with probabilities."
#         )

#     p_cond_col = f"p_{'_'.join(sigma_algebra)}"
#     process.omega[p_cond_col] = process.omega.groupby(sigma_algebra)["p"].transform(
#         lambda x: x / x.sum()
#     )

#     RV_values = RV(process.omega)

#     result = process.omega.groupby(sigma_algebra).apply(
#         lambda g: (RV_values.loc[g.index] * g[p_cond_col]).sum(),
#         include_groups=False,
#     )

#     process.omega.drop(columns=[p_cond_col], inplace=True)
