...
def test_list_with_duplicates_raises():
    with pytest.raises(ValueError, match="index must not contain duplicate values"):
        IndexValidator(indices=[1, 1], name="I")


def test_list_with_duplicate_tuples_raises():
    with pytest.raises(ValueError, match="index must not contain duplicate values"):
        IndexValidator(indices=[(1, 2), (1, 2)], name="I")


def test_pd_index_with_duplicates_raises():
    with pytest.raises(ValueError, match="index must not contain duplicate values"):
        IndexValidator(indices=pd.Index([1, 1]), name="I")


def test_pd_multiindex_with_duplicates_raises():
    with pytest.raises(ValueError, match="index must not contain duplicate values"):
        IndexValidator(indices=pd.MultiIndex.from_tuples([(1, 2), (1, 2)]), name="I")
...
def test_list_with_mixed_tuples_and_non_tuples_raises():
    with pytest.raises(
        ...
        IndexValidator(indices=[(1, 2), 3], name="I")
...
def test_list_with_inconsistent_tuple_length_raises():
    with pytest.raises(ValueError, match="All tuples must have the same length"):
        IndexValidator(indices=[(1, 2), (3, 4, 5)], name="I")
...
def test_variable_names_non_list_raises():
    with pytest.raises(
        ...
        IndexValidator(indices=[1, 2], name="I", variable_names="not a list")
...
def test_variable_names_contains_non_hashable_raises():
    with pytest.raises(
        ...
        IndexValidator(indices=[1, 2], name="I", variable_names=["valid", {}])
...
def test_variable_names_with_duplicates_raises():
    with pytest.raises(
        ...
        IndexValidator(indices=[1, 2], name="I", variable_names=["dup", "dup"])
...
def test_variable_names_inconsistent_with_multi_index_level_names_raises():
    ...
    with pytest.raises(
        ...
        IndexValidator(indices=indices, name="I", variable_names=["wrong", "names"])
...
def test_too_many_variable_names_with_multiindex_raises():
    ...
    with pytest.raises(
        ...
        IndexValidator(
            indices=indices, name="I", variable_names=["num", "letter", "extra"]
        )
...
def test_too_many_variable_names_with_index_raises():
    ...
    with pytest.raises(
        ...
        IndexValidator(indices=indices, name="I", variable_names=["one", "two"])
...
def test_variable_name_inconsistent_with_index_name_raises():
    ...
    with pytest.raises(
        ...
        IndexValidator(indices=indices, name="I", variable_names=["wrong_name"])
