...
from sigalg.core import FeatureEmbedding, SamplePointFeatures


class TestConstructor:
    def test_basic_construction(self):
        features = pd.Series([1, 2, 3], index=["a", "b", "c"], name="omega")
        spf = SamplePointFeatures(name="omega", values=features)
...
class TestProperties:
    ...
    def test_name_property(self):
        ...
        spf = SamplePointFeatures(name="test_name", values=features)
...
class TestIteration:
    ...
    def test_iter_empty(self):
        ...
        spf = SamplePointFeatures(name="empty", values=features)
        values = list(spf)
        assert values == []
...
class TestLen:
    ...
    def test_len_single_feature(self):
        ...
        spf = SamplePointFeatures(name="single", values=features)
        assert len(spf) == 1
...
class TestValidation:
    ...
    def test_features_name_mismatch(self):
        ...
        with pytest.raises(ValueError, match="values.name must match the given name"):
            SamplePointFeatures(name="omega", values=features)
...
class TestEquality:
    def test_equal_spf(self):
        features = pd.Series([1, 2, 3], index=["a", "b", "c"], name="omega")
        spf1 = SamplePointFeatures(name="omega", values=features)
        spf2 = SamplePointFeatures(name="omega", values=features)
        assert spf1 == spf2
    ...
    def test_equal_spf_different_objects_same_values(self):
        ...
        spf1 = SamplePointFeatures(name="omega", values=features1)
        spf2 = SamplePointFeatures(name="omega", values=features2)
        assert spf1 == spf2
    ...
    def test_not_equal_different_names(self):
        ...
        spf1 = SamplePointFeatures(name="omega0", values=features1)
        spf2 = SamplePointFeatures(name="omega1", values=features2)
        assert spf1 != spf2
    ...
    def test_equal_from_feature_embedding(self):
        ...
        spf1 = SamplePointFeatures.from_feature_embedding(
            sample_index=0, feature_embedding=feature_embedding
        )
        spf2 = SamplePointFeatures.from_feature_embedding(
            sample_index=0, feature_embedding=feature_embedding
        )
        assert spf1 == spf2
...
class TestFromFeatureEmbedding:
    ...
    def test_from_feature_embedding_basic(self):
        values = pd.DataFrame([[1, 2], [3, 4], [5, 6]])
        feature_embedding = FeatureEmbedding(values=values, name="X")
        spf = SamplePointFeatures.from_feature_embedding(
            sample_index=0, feature_embedding=feature_embedding
        )
    ...
    def test_from_feature_embedding_string_index_second_row(self):
        ...
        feature_embedding = FeatureEmbedding(values=values, name="features")
        spf = SamplePointFeatures.from_feature_embedding(
    ...
    def test_from_feature_embedding_string_indices_with_custom_names(self):
        ...
        feature_embedding = FeatureEmbedding(values=values, name="data")
        spf = SamplePointFeatures.from_feature_embedding(
