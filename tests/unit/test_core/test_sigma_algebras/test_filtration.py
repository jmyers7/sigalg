...
class TestConstructor:
    ...
    @pytest.fixture
    def trivial_algebra(self, Omega):
        return SigmaAlgebra.trivial(Omega)

    @pytest.fixture
    def middle_algebra(self, Omega):
        atom_ids = {0: 0, 1: 0, 2: 1, 3: 1}
        return SigmaAlgebra(sample_space=Omega).from_dict(sample_id_to_atom_id=atom_ids)

    @pytest.fixture
    def power_set_algebra(self, Omega):
    ...
    def test_constructor_stores_sample_space(
        self, Omega, trivial_algebra, power_set_algebra
    ):
        ...
        assert filtration.sample_space == Omega
...
class TestValidation:
    ...
    def test_invalid_time_dict_instead_of_time_raises_error(self, Omega):
    ...
    def test_invalid_time_string_instead_of_time_raises_error(self, Omega):
    ...
    def test_invalid_name_list_name_raises_error(self, Omega):
    ...
    def test_invalid_name_dict_name_raises_error(self, Omega):
...
class TestDataAccess:
    ...
    @pytest.fixture
    def discrete_filtration(self, Omega):
        trivial = SigmaAlgebra.trivial(Omega)
        atom_ids = {0: 0, 1: 0, 2: 1, 3: 1}
        middle = SigmaAlgebra(sample_space=Omega).from_dict(
            sample_id_to_atom_id=atom_ids
    ...
    @pytest.fixture
    def continuous_filtration(self, Omega):
        trivial = SigmaAlgebra.trivial(Omega)
        atom_ids = {0: 0, 1: 0, 2: 1, 3: 1}
        middle = SigmaAlgebra(sample_space=Omega).from_dict(
            sample_id_to_atom_id=atom_ids
    ...
    def test_at_exact_time_discrete_first_time_point(self, discrete_filtration, Omega):
        """Test accessing sigma algebra at first time point in discrete filtration."""
        time_value = 0
        alg = discrete_filtration.at[time_value]
        expected = SigmaAlgebra.trivial(Omega)
        assert alg == expected

    def test_at_exact_time_discrete_second_time_point(self, discrete_filtration, Omega):
        """Test accessing sigma algebra at second time point in discrete filtration."""
        time_value = 1
        alg = discrete_filtration.at[time_value]
        atom_ids = {0: 0, 1: 0, 2: 1, 3: 1}
        expected = SigmaAlgebra(sample_space=Omega).from_dict(
            sample_id_to_atom_id=atom_ids
        )
        assert alg == expected

    def test_at_exact_time_discrete_last_time_point(self, discrete_filtration, Omega):
        """Test accessing sigma algebra at last time point in discrete filtration."""
        time_value = 2
        alg = discrete_filtration.at[time_value]
        expected = SigmaAlgebra.power_set(Omega)
        assert alg == expected

    def test_at_exact_time_continuous_first_time_point(
        self, continuous_filtration, Omega
    ):
    ...
    def test_at_interpolated_time_discrete_between_first_and_second(
        self, discrete_filtration, Omega
    ):
...
class TestSequenceMethods:
    ...
    def test_iteration_yields_all_sigma_algebras(self, filtration, Omega):
        """Test that iteration yields all sigma algebras in order."""
        algebras = list(filtration)
        assert len(algebras) == 3
        assert algebras[0] == SigmaAlgebra.trivial(Omega)
        assert algebras[2] == SigmaAlgebra.power_set(Omega)
