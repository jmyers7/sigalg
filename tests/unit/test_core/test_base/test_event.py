...
class TestLen:
    ...
    def event(self):
        ...
        return Event(space, ["omega0", "omega1", "omega2"])
...
class TestSetTheoreticOperations:
    ...
    def test_union_different_spaces_raises_error(self):
        ...
        event1 = Event(space1, ["omega0"])
        event2 = Event(space2, ["a"])
    ...
    def test_intersection_disjoint(self, sample_space):
        event_A = Event(sample_space, ["omega0", "omega1"])
        event_B = Event(sample_space, ["omega2", "omega3"])
        intersection = event_A & event_B
...
class TestSubsetSuperset:
    ...
    def test_subset_proper(self, sample_space):
        event_A = Event(sample_space, ["omega0", "omega1"])
        event_B = Event(sample_space, ["omega0", "omega1", "omega2"])
        assert event_A <= event_B
        assert event_A < event_B

    def test_subset_equal(self, sample_space):
        event_A = Event(sample_space, ["omega0", "omega1"])
        event_B = Event(sample_space, ["omega0", "omega1"])
        assert event_A <= event_B
        assert not (event_A < event_B)

    def test_not_subset(self, sample_space):
        event_A = Event(sample_space, ["omega0", "omega2"])
        event_B = Event(sample_space, ["omega0", "omega1"])
        assert not (event_A <= event_B)
        assert not (event_A < event_B)

    def test_superset_proper(self, sample_space):
        event_A = Event(sample_space, ["omega0", "omega1", "omega2"])
        event_B = Event(sample_space, ["omega0", "omega1"])
        assert event_A >= event_B
        assert event_A > event_B
    ...
    def test_all_superset_of_all(self, sample_space):
        full = Event(sample_space, list(sample_space))
        event = Event(sample_space, ["omega0", "omega1"])
        assert full >= event
        assert full > event
...
class TestEquality:
    ...
    def test_equality_same_indices(self, sample_space):
        event1 = Event(sample_space, ["omega0", "omega1"])
        event2 = Event(sample_space, ["omega0", "omega1"])
        assert event1 == event2

    def test_equality_different_indices(self, sample_space):
        event1 = Event(sample_space, ["omega0", "omega1"])
        event2 = Event(sample_space, ["omega0", "omega2"])
        assert event1 != event2
