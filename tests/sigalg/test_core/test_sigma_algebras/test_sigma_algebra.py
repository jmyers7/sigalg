...
class TestConstructor:
    ...
    def test_from_dict_integer_atom_ids_default_name(self, Omega):
        ...
        assert F.domain == Omega
        assert F.point_to_atom_id == atom_ids
        assert F.name == "F"
    ...
    def test_from_dict_string_atom_ids_custom_name(self, Omega):
        ...
        G = SigmaAlgebra(domain=Omega, name="G", mapping=atom_ids)

        assert G.domain == Omega
        assert G.point_to_atom_id == atom_ids
        assert G.name == "G"
    ...
    def test_index_of_data_is_domain_data(self):
        """Test that the index of the data attribute is the underlying data of the domain."""
        ...
        assert F.data.index is Omega.data
...
class TestPowerSet:
    def test_power_set_three_samples_default_name(self):
        ...
        for idx, sample_id in enumerate(Omega.data):
            assert power_set.point_to_atom_id[sample_id] == idx
    ...
    def test_power_set_four_samples_custom_name(self):
        ...
        assert G.domain == Omega
        for idx, sample_id in enumerate(Omega.data):
            assert G.point_to_atom_id[sample_id] == idx
    ...
    def test_data_index_is_domain_data(self):
        X = Domain([(1, 2), (3, 4)])
        R = SigmaAlgebra.power_set(X)

        assert R.data.index is X.data
...
class TestTrivial:
    ...
    def test_trivial_creation_four_samples_custom_name(self):
        ...
        assert G.domain == Omega
        unique_atom_ids = set(G.point_to_atom_id.values())
        assert len(unique_atom_ids) == 1
...
class TestDomain:
    ...
    def test_domain_is_copy_but_data_is_not(self, Omega, atom_ids):
        """The domain is a copy of the original, but its underlying data is the index of the underlying data of the sigma-algebra."""
        F = SigmaAlgebra(domain=Omega, mapping=atom_ids)

        assert F.domain is not Omega
        assert F.data.index is Omega.data
...
class TestAtomIdDictionaries:
    ...
    def test_atom_id_to_points(self, F):
        """Test that atom_id_to_points returns correct mapping."""
        expected = {
            0: [0, 1],
            1: [2, 3],
        }

        assert F.atom_id_to_points == expected
...
class TestGetSet:
    ...
    def test_get_set_custom_name(self, F):
        ...
        event = F.get_set([0, 1], name="CustomMeasurableSet")

        assert event.name == "CustomMeasurableSet"

    def test_get_set_default_name(self, F):
        """Test get_set with default name."""
        event = F.get_set([2, 3])
...
class TestIsMeasurable:
    ...
    def test_is_measurable_with_nonmeasurable_list_of_indices(self, F):
        ...
        assert not F.is_measurable([2, 3])
...
class TestIteration:
    ...
    def test_iteration_yields_tuples(self, F):
        """Test that iterating over the SigmaAlgebra yields MeasurableSet instances."""
        for atom in F:
            assert isinstance(atom, Set)

    def test_iteration_covers_all_atoms(self, F):
        """Test that iteration covers all atom IDs."""
        atom_ids_seen = set()
        for atom in F:
            atom_ids_seen.add(atom.name)
...
class TestEquality:
    def test_non_equality_different_atom(self):
        ...
        Omega = SampleSpace.from_sequence(size=3)
        F1 = SigmaAlgebra(domain=Omega, mapping={0: 0, 1: 0, 2: 1})
        F2 = SigmaAlgebra(domain=Omega, mapping={0: 0, 1: 1, 2: 1})
    ...
    def test_non_equality_different_sample_spaces(self):
        ...
        F1 = SigmaAlgebra(domain=Omega1, mapping={0: 0, 1: 0})
        F2 = SigmaAlgebra(domain=Omega2, mapping={"a": 0, "b": 0})
    ...
    def test_non_equality_wrong_type_string(self):
        ...
        Omega = SampleSpace.from_sequence(size=3)
        F = SigmaAlgebra(domain=Omega, mapping={0: 0, 1: 0, 2: 1})
    ...
    def test_equality_same_components_different_names(self):
        ...
        Omega = SampleSpace.from_sequence(size=3)
        F1 = SigmaAlgebra(domain=Omega, name="F1", mapping={0: 0, 1: 0, 2: 1})
        F2 = SigmaAlgebra(domain=Omega, name="F2", mapping={0: 0, 1: 0, 2: 1})
...
class TestJoin:
    ...
    def test_join_with_different_sample_spaces_raises_error(self, F1):
        ...
        with pytest.raises(
            ValueError, match="All sigma-algebras must have the same domain"
        ):
...
class TestOrderRelations:
    ...
    def test_le_trivial_and_power_set(self, Omega):
        ...
        trivial = SigmaAlgebra.trivial(domain=Omega)
        power_set = SigmaAlgebra.power_set(domain=Omega)
    ...
    def test_le_coarser_and_finer(self, Omega):
        ...
        F_coarse = SigmaAlgebra(domain=Omega, mapping=coarse_atom_ids)
        fine_atom_ids = {0: 0, 1: 0, 2: 1, 3: 2}
        F_fine = SigmaAlgebra(domain=Omega, mapping=fine_atom_ids)
    ...
    def test_le_transitive(self):
        ...
        Omega = SampleSpace.from_sequence(size=4)
        F1 = SigmaAlgebra.trivial(domain=Omega)
        B_atom_ids = {0: 0, 1: 0, 2: 1, 3: 1}
        F2 = SigmaAlgebra(domain=Omega, mapping=B_atom_ids)
        F3 = SigmaAlgebra.power_set(domain=Omega)
    ...
    def test_lt_not_proper_when_equal(self, Omega):
        ...
        F = SigmaAlgebra.trivial(domain=Omega)
    ...
    def test_incomparable_sigma_algebras(self, Omega):
        ...
        F1 = SigmaAlgebra(domain=Omega, mapping=sigma1_atom_ids)
        sigma2_atom_ids = {0: 0, 1: 1, 2: 0, 3: 1}
        F2 = SigmaAlgebra(domain=Omega, mapping=sigma2_atom_ids)
    ...
    def test_three_level_chain(self):
        ...
        middle = SigmaAlgebra(domain=Omega, mapping=middle_atom_ids)
        power_set = SigmaAlgebra.power_set(domain=Omega)
    ...
    def test_antisymmetry(self, Omega):
        ...
        atom_ids = {0: 0, 1: 0, 2: 1, 3: 1}
        F1 = SigmaAlgebra(domain=Omega, mapping=atom_ids)
        F2 = SigmaAlgebra(domain=Omega, mapping=atom_ids)
